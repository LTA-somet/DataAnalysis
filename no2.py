...
# Đọc dữ liệu từ tệp và tạo DataFrame
df = pd.read_csv("WineQT.csv", delimiter=",")  # đọc dữ liệu  , sử dụng dấu , làm ký tự ngăn cách
...
data_after_preprocessing.to_csv("WineQT_after_preprocessing.csv", index=False)
...
def linear_regression_model(data_after_preprocessing):
    ...
    X = data_after_preprocessing['volatile acidity']  # biến độc lập
    y = data_after_preprocessing['quality']  # biến phụ thuộc
